"""
Example of using MCP client to connect to MCP servers
and use their tools with Mellea, with session management using contextlib.

Supports both streamable-http and SSE transports:

1. **streamable-http**: Bidirectional HTTP streaming (used by GitHub Copilot MCP)
   - Best for: Production APIs, cloud services
   - Example: GitHub Copilot MCP server
   
2. **sse**: Server-Sent Events (unidirectional streaming)
   - Best for: Simple streaming, local servers
   - Example: Local MCP servers, development environments

Usage:
    # For streamable-http (GitHub Copilot)
...
async def mcp_session_manager(
    ...
    try:
        ...
        if transport == "streamable-http":
            ...
            raise ValueError(f"Unknown transport: {transport}. Must be 'streamable-http' or 'sse'")
...
async def _initialize_session(manager: MCPSessionManager, session: ClientSession, transport: str):
...
async def main_async():
    ...
    # Choose transport: "streamable-http" or "sse"
    transport = "streamable-http"  # Change to "sse" for SSE transport
    ...
    try:
        ...
        async with mcp_session_manager(
            ...
            output = m.instruct(
                "Get information about the current GitHub user. Use available guthub tool 'get_me' to fetch their profile information.",
                model_options={
