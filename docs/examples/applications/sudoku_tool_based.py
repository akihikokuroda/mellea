...
def extract_numbers_from_colored_grid(image_path: str) -> list[list[int]]:
    ...
    Raises:
        ValueError: If image cannot be read or sudoku grid cannot be detected
    """
    # Read image in color
...
def generate_and_execute_sudoku_solver_with_tools(
    ...
    Raises:
        ValueError: If code generation fails, solution verification fails, or user declines execution
    """
...
def main_sync() -> None:
    ...
    Raises:
        SystemExit: If no image path is provided or an error occurs during solving
    """
    if len(sys.argv) < 2:
